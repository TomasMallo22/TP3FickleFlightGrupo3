package com.example.tp3fickleflightgrupo3.holders

import android.view.View
import android.widget.ImageView
import android.widget.TextView
import androidx.recyclerview.widget.RecyclerView
import com.bumptech.glide.Glide
import com.example.tp3fickleflightgrupo3.R
import com.example.tp3fickleflightgrupo3.data.model.Offer

class OfferHolder(view: View) : RecyclerView.ViewHolder(view) {
    private val image: ImageView = view.findViewById(R.id.offer_image)
    private val descriptionLong: TextView = view.findViewById(R.id.offer_description_long)
    private val details: TextView = view.findViewById(R.id.offer_details)
    private val heartIcon: ImageView = view.findViewById(R.id.heart_icon)

    fun bind(offer: Offer) {
        descriptionLong.text = offer.descriptionLong
        details.text = offer.details
        Glide.with(image.context).load(offer.imageResId).into(image)

        heartIcon.setOnClickListener {
            heartIcon.isSelected = !heartIcon.isSelected
        }
    }
}
